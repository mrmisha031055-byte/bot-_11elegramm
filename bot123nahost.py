...
Дата: 2026-03-22
Исправлены: клавиатура, отправка поддержки, обработка отчетов
"""
...
def get_report_keyboard(day: int) -> InlineKeyboardMarkup:
    """Клавиатура для отчета в зависимости от дня с передачей дня в callback_data"""
...
async def process_report(callback: types.CallbackQuery):
    """Обработка отчетов - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    user_id = callback.from_user.id
    report_data = callback.data.split('_')
    
    # ИСПРАВЛЕНО: правильно определяем отчет и день
    # Формат: report_5/5_1 -> report_data = ['report', '5/5', '1']
    if len(report_data) >= 3:
        report = report_data[1]  # Это "5/5" или "3-4/5"
        reported_day = int(report_data[2])  # Это день
    else:
    ...
    # ИСПРАВЛЕНО: используем правильный ключ report (без дня)
...
async def admin_user_info(message: types.Message):
    ...
    try:
        ...
        if reports:
            ...
            info_text += f"*Последние 10 отчеты:*\n"
